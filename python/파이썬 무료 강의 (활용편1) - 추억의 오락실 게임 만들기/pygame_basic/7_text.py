...
# 배경 이미지 불러오기
background = pygame.image.load("D:/Code/인프런/python/game/pygame_basic/background.png")

# 캐릭터(스프라이트) 불러오기
character = pygame.image.load("D:/Code/인프런/python/game/pygame_basic/character.png")
...
# 적(enemy) 캐릭터
enemy = pygame.image.load("D:/Code/인프런/python/game/pygame_basic/enemy.png")
...
while running:
    ...
    for event in pygame.event.get(): # 이벤트가 발생했는가?
        ...
        if event.type == pygame.KEYDOWN: # 키가 눌렸는지 확인
            if event.key == pygame.K_LEFT: # 캐릭터를 왼쪽으로
                ...
                to_x += character_speed 
            elif event.key == pygame.K_UP: # 캐린턱를 위로
    ...
    # 타이머 집어 넣기
    # 경과 시간 계산
    # 경과 시간(ms)을 100으로 나누어서 초 단위로 표시
    elapsed_time = (pygame.time.get_ticks() - start_ticks) / 1000

    # 출력할 글자, True, 글자색상(R, G, B)
